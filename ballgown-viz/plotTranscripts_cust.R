plotTranscripts_cust = function(gene, gown, samples=NULL, colorby='transcript',
  ...
  if(gown@RSEM & colorby=='exon'){
    stop('RSEM objects do not yet have exon-level measurements,
  ...
  if(!all(samples %in% sampleNames(gown))){
    stop('all entries of "samples" must be part of "gown". Use
  ...
  if(!is.null(customOrder)){
    if(!all(sort(customOrder) == sort(thetranscripts))){
      stop('customOrder must include each transcript in gene
                exactly once.')
    }
  }
  ...
  for(s in 1:n){
    ...
    plot(xax, rep(0,length(xax)), ylim=c(0,ymax), 
         type="n", xlab="Genomic Position", yaxt = "n", ylab="", cex.main=main_size, cex.axis = 1, font = 2, font.lab = 2, cex.lab = 1.2)
    if(n > 1){
      title(samples[s], cex.main=main_size)
    }
    ...
    if(legend){
      ...
      for(i in 1:length(colscale)){
        polygon(x=c(leglocs[i], leglocs[i], leglocs[i+1], leglocs[i+1]),
                y=c(ymax-0.5, ymax-0.2, ymax-0.2, ymax-0.5), border=NA, 
      ...
      text(x = seq(min(xax)+1, max(xax)-1, length=10), 
           ...
           cex=1, cex.axis = 2) 
      if(log){
        text(x=median(xax), y=ymax-0.7, 
             labels=paste("log expression, by",  colorby), cex=1)
      }else{
        text(x=median(xax), y=ymax-0.7, 
             labels=paste("Expression, by",  colorby), cex=1)
      }
    }
    
    # label the transcripts on the y-axis (if asked for)
    if(labelTranscripts){
      axis(side=2, at=c(1:numtx), labels=transcript_loop, font.axis=2, 
           las=1)
    }
  ...
  # put title on plot
  if(!is.null(main)){
    title(main, outer=(n>1), cex.main=main_size)
  }else{
    if(n==1){
      title(paste0(gene,': ',samples), cex.main=main_size)
    }else{
      title(gene, outer=TRUE, cex.main=main_size)
    }
  }
